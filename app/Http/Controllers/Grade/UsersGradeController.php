...
class UsersGradeController extends Controller
{
    // Просмотр списка студентов определенной группы(по $id_grade)
    public function index($id_grade)
    {
        $name = DB::table('grades')->where('id', '=', $id_grade)->value('name');

        $users = DB::table('users')
            ->where('users.id', '>', '1')
            ->where('grades_users.grades_id', '=', $id_grade)
            ->join('grades_users', 'users.id', '=', 'grades_users.users_id')
            ->get();
        $i = 1;

        return view('grade.users', ['user' => $users, 'id_grade' => $id_grade, 'name' => $name, 'i' => $i]);
    }

    // Просмотр списка студентов определенной группы(по $id_grade)
    public function option($id_student)
    {
        $users = DB::table('users')
            ->where('users.id', '=', $id_student)
            ->first();
        return view('auth.option_users', ['user' => $users]);
    }
